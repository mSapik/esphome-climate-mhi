...
class MhiClimate : public climate_ir::ClimateIR {
 ...
 protected:
  /** Скорректировать перечень поддерживаемых fan_modes */
  climate::ClimateTraits traits() override {
    auto traits = climate_ir::ClimateIR::traits();
    // Формируем список по количеству уровней
    std::vector<climate::ClimateFanMode> modes;
    modes.push_back(climate::CLIMATE_FAN_AUTO);
    modes.push_back(climate::CLIMATE_FAN_LOW);
    if (fan_levels_ == FAN_LEVELS_4)
      modes.push_back(climate::CLIMATE_FAN_MIDDLE);
    modes.push_back(climate::CLIMATE_FAN_MEDIUM);
    modes.push_back(climate::CLIMATE_FAN_HIGH);
    // Всегда доступны FOCUS и DIFFUSE
    modes.push_back(climate::CLIMATE_FAN_FOCUS);
    modes.push_back(climate::CLIMATE_FAN_DIFFUSE);
    traits.set_supported_fan_modes(modes);
    return traits;
  }
